...
// Try to augment DATA with file-based dataset generated by scraper
try {
  const filePath = path.join(process.cwd(), 'data', 'market_rates.json');
  if (fs.existsSync(filePath)) {
    const raw = fs.readFileSync(filePath, 'utf-8');
    const parsed: MarketRate[] = JSON.parse(raw);
    DATA.push(...parsed);
  }
} catch (err) {
  // silently ignore errors (e.g., file missing on first run)
}
